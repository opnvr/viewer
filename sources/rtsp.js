...
const factory = (config, sourceConfig, server) => {
  ...
  function start () {
    ...
    const command = ffmpeg()
      ...
      .on('error', function (err) {
        log.debug('An error occurred: ' + err.message)
      })
      .on('end', function () {
    ...
    ffstream.on('data', function (data) {
      if (!moovAtom) {
        ...
        initFragment(rtspConfig.id, data.buffer.slice(data.byteOffset, data.byteOffset + data.byteLength))
  ...
  function parseMP4 (camera, dataView, offset, size) {
    ...
    while (offset < size) {
      ...
      if (type === MP4_FTYP) {
        log.debug(`CAM${camera.toString().padStart(2, '0')} Got MP4_FTYP`, offset, len)
        ftypAtom = new Uint8Array(dataView.buffer.slice(offset, offset + len))
      } else if (type === MP4_MOOV) {
        log.debug(`CAM${camera.toString().padStart(2, '0')} Got MP4_MOOV`, offset, len)
