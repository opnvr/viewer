...
const factory = () => {
  ...
  function start(config, sourceConfig, itemConfig, server, mqttClient) {
    const log = require("loglevel").getLogger(
      "notifications:frigate-" + sourceConfig.id,
    );
    log.debug("Start");
    ...
    const { value: notificationConfig, error } =
      schema.validate(inheritedConfig);
    if (error) {
      throw new Error(
        "Invalid configuration for notifications:frigate, " + error.message,
      );
    }
    ...
    mqttClient.on("message", (topic, message) => {
      if (topic.includes("frigate/events")) {
        ...
        if (
          data.after.camera === notificationConfig.camera &&
          !data.after.stationary
        ) {
